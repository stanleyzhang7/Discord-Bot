...
function isGameOver() {
    ...
    if (tictactoe_state[1][1] != EMPTY) {
        if ((tictactoe_state[0][0] == tictactoe_state[1][1] && tictactoe_state[1][1] == tictactoe_state[2][2]) || (tictactoe_state[0][2] == tictactoe_state[1][1] && tictactoe_state[1][1] == tictactoe_state[2][0])) {
            return true;
        } 
    }
    return false;
...
client.on('interactionCreate', async interaction => {
    ...
    // if the game was already finished, grid should not be clickable
    if (isGameOver()) {
        interaction.update({
            components: makeGrid()
        })
    ...
    if (tictactoe_state[row][col] != EMPTY) {
        interaction.update({
            content: "You cannot select that square!",
            components: makeGrid()
        })
        return;
    }
    ...
    // check if game is a draw or won after player move
    if (isGameOver()) {
        ...
        interaction.update({
            content: "You've won a game! Current wins: " + (user.get('score') + 1),
            components: makeGrid()
        })
        return;
    }
    ...
    // check if game is a draw or won after bot move
    if (isGameOver()) {
        interaction.update({
            content: "You lost!",
            components: makeGrid()
